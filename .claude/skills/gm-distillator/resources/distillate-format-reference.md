...
platforms with no shared methodology layer. GOMAD is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, GOMAD offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: GOMAD = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); GOMAD maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is GOMAD's whitespace
```
...
**Brief says:**
```
gomad-help must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
gomad-help must always be included as a base skill in every bundle/install
(solves discoverability problem)
```

**Distillate keeps the more contextual version:**
```
- gomad-help: always included as base skill in every bundle (solves discoverability)
```
...
type: gomad-distillate
sources:
  - "product-brief-gomad-next-gen-installer.md"
  - "product-brief-gomad-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
## Core Concept
- GOMAD Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing GOMAD methodology across 40+ AI platforms
- Three layers: self-describing plugins (gomad-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via gomad-setup skill
- Transforms GOMAD from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + gomad-manifest.json extending for GOMAD-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"gomad","replaces-skill":"gomad-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- gomad-setup: global skill scanning installed gomad-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- gomad-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
## Differentiation
- Anti-fragmentation: GOMAD = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by GoMad + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via GOMAD Builder (separate initiative)
...
- Future plugin creators: non-dev authors using GOMAD Builder; need distribution without building own installer
...
- gomad-setup discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- gomad-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, gomad-setup, gomad-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: GOMAD Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/installer/gomad-cli.js` (Commander.js) → `tools/installer/core/installer.js`
...
- Skills already use directory-per-skill layout; gomad-manifest.json sidecars exist but are not source of truth
...
- gomad-update mechanics: diff/replace? Preserve user customizations?
...
- gomad-manifest.json as open standard submission to Agent Skills governance?
...
## Opportunities
- Module authors as acquisition channel: each published plugin distributes GOMAD to creator's audience
- CI/CD integration: gomad-setup as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing GOMAD modules with third-party skills for custom methodology stacks
