...
> **[遗留自 1-5-multitalk-single-card-happy-path]**: `wan/modules/attention.py` 的 `flash_attention()` SDPA fallback 路径在 NPU 上仅对 `is_causal=False` + 无 mask + uniform-length batch 安全（依据 Ascend `FlashAttentionScore` 文档：`torch.nn.functional.scaled_dot_product_attention` 在 NPU 上已被 patch 为 FA 自动 dispatch）。**未来需要直接切 `torch_npu.npu_fusion_attention`** 的场景：(a) 任何 `causal=True` 的 caller — NPU SDPA causal 路径有限制；(b) batch>1 异长序列的正确 padding mask；(c) `flash_attention` 的 `q_lens`/`k_lens` 参数被实际使用时（当前 SDPA fallback 直接丢弃这些，与原 wrapper 的 padding-warning 一致）。此项可与 Story 1.4 attention_dispatch.py 的 BNSD 路径合并：把 `flash_attention` SDPA fallback 升级为通过 `dispatch_memory_efficient_attention` 走 npu_fusion_attention（TND layout），统一所有 attention 路径走 NPU 原生融合算子。预计在 Phase 2 / Epic 4 attention 优化阶段处理。 — LOW (J1 happy path 不受影响)

> **[遗留自 1-5-multitalk-single-card-happy-path]**: J1 真机第二次撞 `decord` 不可装 — upstream PyPI 的 `decord` 没有 aarch64 wheel（CANN docker host 几乎都是 aarch64），且 host Python 版本 < 3.11 进一步收窄候选。`wan/utils/utils.py:15` 是顶层 `from decord import VideoReader, cpu`，被 `generate_infinitetalk.py` / `app.py` / `wan/multitalk.py` 强引入，不可跳过。推荐解法：用社区 fork `eva-decord`（drop-in 同 namespace 替换），把 `requirements.txt` 里 `decord` 行替换为 `eva-decord`（或注释 `decord` 后单独 `pip install eva-decord`）。Story 1.7 README-NPU.md MUST 写清 NPU host 的 decord 替换决策；同时考虑是否把 `eva-decord` 写进 `requirements-npu.txt` 而非改动上游 `requirements.txt`（保护 NFR-04 上游 rebase 表面）。 — MEDIUM (升级原因: 同上 — 阻塞 J1 acceptance evidence 收集直到用户 manual fix)
